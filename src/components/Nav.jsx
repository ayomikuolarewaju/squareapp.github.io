...
function Nav() {
  ...
  return (
      <div className={
          url === '/' ?
              "bg-[url('../../images/banner-bg.png')]  bg-no-repeat bg-cover h-[700px] mx-auto w-full px-4 sm:px-8 md:px-16 xl:px-32 2xl:px-64 flex justify-between text-white gap-3 items-start"
              :
          ...
          <div className='w-1/2 flex items-center justify-start h-[80px] pt-2'>
           ...
           <div className='flex  flex-col justify-between gap-y-5 h-[80px] items-center w-1/2'>
              
              <div className='cursor-pointer w-[60px] h-[60px] ' onClick={() => setIsshow(!isShow)}>
              ...
              <div className='flex flex-col gap-y-5 md:w-[300px] font-bold bg-[#B3973D] h-fit items-center relative p-10 rounded-lg'>
                      {menu.map((m, index) => <div key={index} onClick={()=>increaseno(index)}> 
                          {
                              no === index ?
                                  <Link className='text-[#420606]' href={m.href}>{m.title}</Link>
                                  :
                                  <Link className='text-white' href={m.href}>{m.title}</Link>
                          }
                      </div>)}
